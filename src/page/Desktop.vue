      ...
      <div class="check-list row">
        <div class="col-md-12">
          ...
          <div class="check-item">
            ...
            <ul class="check list-inline">
              {{locationText}}
              <li :class="{selected: locationChecked(item)}"
                  v-for="(item, index) in locationList" :key="index">
                <h6 @click="selected( 'Location', locationList, item, index)" v-text="getLocationText(item)"></h6>
              </li>
            </ul>
            <ul v-if="location&&location.showSuburb&&location.text!='All'" class="suburb list-inline">
              <li :class="{selected: locationSuburbChecked(location, 'CBD')}" @click="selectLocation(location, 'CBD', 0)">
              <h6>CBD</h6></li>
              <li :class="{selected: locationSuburbChecked(location, 'Suburb2')}"
              @click="selectLocation(location, 'Suburb2', 1)"><h6>Suburb2</h6></li>
              <li :class="{selected: locationSuburbChecked(location, 'Suburb3')}"
              @click="selectLocation(location, 'Suburb3', 2)"><h6>Suburb3</h6></li>
              <li :class="{selected: locationSuburbChecked(location, 'Suburb4')}"
              @click="selectLocation(location, 'Suburb4', 3)"><h6>Suburb4</h6></li>
              <li :class="{selected: locationSuburbChecked(location, 'Suburb5')}" @click="selectLocation(location, 'Suburb5', 4)">
              <h6>Suburb5</h6></li>
              </ul>
        </div>
        <div class="clearfix"></div>
        ...
        <div class="show-all-options">
          <h5 @click="showAllOptions=!showAllOptions">Show all options</h5>
        </div>
      </div>
    </div>
    <div class="card-list">
      ...
      <div>
        <div class="row">
          <div class="card col-xs-6 col-md-3" v-for="(item, index) in cardList" :key="index">
            <div>
...
//    props: ['locationList', 'preferenceList', 'nationalityList', 'massage', 'age'],
  ...
  computed: {
    ...
    locationText() {
      this.locationTexts = {}
      this.filters['Location'].forEach ((v) => {
        let filter = v.filterName.split(',')
        if (!this.locationTexts[filter[0]]) {
          this.locationTexts[filter[0]] = []
        }

        this.locationTexts[filter[0]].push(filter[1])
      })
    },
    finalFilters () {
      let finalFilters = []
  ...
  methods: {
    ...
    selected (type, items, item, index) {
      if (type == 'Location') {
        ...
        // close all Suburb
        this.locationList.map((v)=>{
          if (v.text != item.text) v.showSuburb = false
        })
    ...
    selectLocation (item, suburb, index) {
      ...
      if (suburb == 'All') {
        ...
        if (i != -1) {
          ...
          let p_index = this.locationList.findIndex((v)=>{
    ...
    locationSuburbChecked (item, suburb) {
      if (suburb == 'All') {
    ...
    locationChecked(item) {
      if (item.text == 'All') {
        return -1 == this.filters['Location'].findIndex ((v) => {
          //return v.filterName.indexOf (item.text) != -1
          return !this.locationList.some((v1)=>{
            return v.filterName.indexOf (item.text) != -1
          })
        })
      }
...
//      console.log(this.locationTexts)
      let finalText = item.text,
          suburbs = this.locationTexts[item.text]
      if (suburbs && !item.showSuburb) {
        finalText +=  ',' + suburbs.join(',')
      }
...
.desktop section .filter .filter-info {
  display: inline;
}
...
.desktop section .check-list .suburb li {
  margin: 0 .4rem;
  cursor: pointer;
}

.desktop section .check-list .check li.selected,
.desktop section .check-list .suburb li.selected {
  color: #B297FF;
}
...
.desktop section .check-list .suburb {
  margin-left: 2rem;
  padding-left: 1rem;
  color: #c6c6c6;
}
